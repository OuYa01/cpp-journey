...
/**
 * CheckIfClientExist - check if client exist in the vectore that have all structure of all client
 * 
 * @AllClientsData: Referance to Clients Data to search in it
 * 
 * Return: True if found otherwise Flase
 */
bool CheckIfClientExist(vector<stClient>& AllClientData)
{
    for (stClient tempCLient : AllClientData)
    {
        if (CurrentClient.AccountNumber == tempCLient.AccountNumber) 
        {
            if (CurrentClient.PinCode == tempCLient.PinCode)
            {
...
/**
 * GetClientInfos - Helper function of login function, it get infos from client to pass them in login function
 */
void GetClientInfos()
{
